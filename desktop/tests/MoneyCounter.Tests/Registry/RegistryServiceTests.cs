...
public sealed class RegistryServiceTests : IAsyncLifetime
{
    ...
    public async Task SameOperationAndPayloadReturnsOriginalReceipt()
    {
        ...
        var first = await _service.CreateModelAsync(command);
        var retry = await _service.CreateModelAsync(command);
    ...
    public async Task RetryReturnsOriginalSnapshotAfterEntityChanges()
    {
        ...
        var first = (await _service.CreateModelAsync(command)).Value!;
        Assert.True((await _service.UpdateModelAsync(new(Guid.NewGuid(), first.Id, first.Revision, "康艺", "JBYD-snapshot", 2000, "changed"))).IsSuccess);

        var retry = (await _service.CreateModelAsync(command)).Value!;
    ...
    {
        var operationId = Guid.NewGuid();
        await _service.CreateModelAsync(new(operationId, "A", "M1", 1, ""));

        var result = await _service.CreateModelAsync(new(operationId, "A", "M2", 1, ""));
    ...
    public async Task StaleRevisionCannotOverwriteModel()
    {
        ...
        var updated = await _service.UpdateModelAsync(new(Guid.NewGuid(), created.Id, created.Revision, "A", "M", 200, "new"));

        var stale = await _service.UpdateModelAsync(new(Guid.NewGuid(), created.Id, created.Revision, "A", "M", 300, "stale"));
    ...
    public async Task ReferencedModelCannotBeDeactivated()
    {
        ...
        var result = await _service.DeactivateModelAsync(new(Guid.NewGuid(), model.Id, model.Revision));
    ...
    public async Task ListsArePagedAndDetailsSurviveReopen()
    {
        ...
        var firstPage = await _service.ListModelsAsync(new(Page: 1, PageSize: 50));
        ...
        await _store.InitializeAsync();
        _service = new SqliteRegistryService(_store);
        Assert.True((await _service.GetModelAsync(id)).IsSuccess);
    }

    private Task<Result<ModelDetail>> CreateModelAsync(string manufacturer, string modelName) =>
        _service.CreateModelAsync(new(Guid.NewGuid(), manufacturer, modelName, 1000, ""));

    private Task<Result<DeviceDetail>> CreateDeviceAsync(string assetCode, long modelId) =>
        _service.CreateDeviceAsync(new(Guid.NewGuid(), assetCode, modelId, null, null, "", "", ""));
}
